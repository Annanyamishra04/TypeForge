...
function ChartTooltip({ active, payload, colors }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-1 font-semibold">{point.accuracy}% accuracy</div>
    </div>
  );
}

/**
 * Renders the authenticated user's actual accuracy trend. Same data
 * source as WpmChart (GET /api/results/me/analytics) — no invented
 * smoothing or synthetic points.
 */
export default function AccuracyChart({ trend }) {
  ...
  if (!trend || trend.length === 0) {
    return (
      <div className="flex h-64 items-center justify-center rounded-lg border border-dashed border-border-strong bg-surface/30 text-sm text-text-tertiary">
        No accuracy data for this period yet.
      </div>
    );
  }
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={trend} margin={{ top: 8, right: 12, bottom: 0, left: -12 }}>
          ...
          <YAxis
            ...
            tickLine={false}
            width={36}
            domain={isSinglePoint ? [Math.max(0, trend[0].accuracy - 10), 100] : [0, 100]}
          />
          ...
          <Line
            ...
            strokeWidth={2}
            dot={{ r: isSinglePoint ? 5 : 3, fill: colors.success, strokeWidth: 0 }}
            activeDot={{ r: 5 }}
